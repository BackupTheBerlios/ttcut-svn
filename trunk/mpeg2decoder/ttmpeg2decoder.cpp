...
bool TTMpeg2Decoder::openMPEG2File(QString cFName, __attribute__ ((unused))long lOffset, __attribute__ ((unused))long lSize)
{
  ...
  if ( mpeg2Stream == NULL )
  {
     mpeg2Stream = new TTFileBuffer( mpeg2FileName.toAscii().constData(), fm_open_read );
	...
	mpeg2StreamOK = true;
     ...
     mpeg2StreamOK = mpeg2Stream->openFile( mpeg2FileName.toAscii().constData(), fm_open_read );
