...
bool TTFileBuffer::openFile( const char* f_name, int f_mode)
{
  ...
  {
   case fm_create :
     file_handle   = _open( file_name, _O_RDWR | _O_CREAT, 0644);
     stream_length = 0;
     break;

   case fm_open_write :
     file_handle = _open( file_name, _O_WRONLY | _O_CREAT, 0644);
     if ( file_handle > -1  )
   ...
   case fm_open_read :
     file_handle = _open( file_name, _O_BINARY | O_RDONLY);
     if ( file_handle > -1  )
       stream_length = streamLength();
     else
       stream_length = 0;
...
off64_t TTFileBuffer::streamLength()
{
  ...
  if ( file_handle > -1 )
  {
    ...
    offset = _lseek( file_handle, (off64_t)0, SEEK_CUR );
    /*set file pointer at end of file*/
    _lseek( file_handle, (off64_t)0, SEEK_END );
    /*gets size of file*/
    o_result = _lseek( file_handle, (off64_t)0, SEEK_CUR );
    /*set the pointer back to previous position*/
    _lseek( file_handle, offset, SEEK_SET );
  }
  else
    o_result = (off64_t)-1;
...
void TTFileBuffer::nextStartCodeTS()
{
  do
  {
    ...
    {
      if( !seekRelative((off64_t)(shift[mem_buffer[buffer_pos+t_delta]])) )
	return;
    }
    --t_index;
    while ( mem_buffer[buffer_pos+t_index] == start_code[t_index] )
    ...
    seekRelative((off64_t)(look_at+shift[mem_buffer[buffer_pos+t_delta+look_at]]));
  }while(-1);
}
...
off64_t TTFileBuffer::directRead( uint8_t* read_buffer, off64_t read_length )
{
  ...
  {
    stream_pos = _lseek( file_handle,(off64_t)0,SEEK_CUR );
    o_result   = _read( file_handle, read_buffer, read_length );
    o_result  += stream_pos;
  }
  else
    o_result = (off64_t)-1;
...
off64_t TTFileBuffer::directWrite( const uint8_t* write_buf, off64_t write_len )
{
  ...
  {
    stream_pos = _lseek( file_handle,(off64_t)0,SEEK_CUR );
    o_result   = _write( file_handle, write_buf, write_len );
    o_result  += stream_pos;
...
void TTFileBuffer::initBuffer()
{
  ...
  if ( file_mode == fm_open_read || file_mode == fm_create )
  {
    ...
    if ( file_handle > -1 )
    {
      stream_pos = _lseek( file_handle, buffer_start, SEEK_SET );
    }
    else
    {
      stream_pos     = 0;
...
void TTFileBuffer::fillBuffer()
{
  ...
  buffer_read_size = _read( file_handle, mem_buffer, buffer_size );
...
bool TTFileBuffer::newPosition( off64_t new_pos )
{
  ...
  else
  {
	 ...
	 new_pos < stream_length + 1) )
    {
      ...
      stream_pos = _lseek(file_handle, (off64_t)new_pos, SEEK_SET);
